...
quad_data <- cyclist[i = !is.na(iso_mean_torque_quad_der_raw),
                     j = list(
                       ...
                       jump_cmj_fuerza_peak
                     ),
                     by = sujetos]

quad_model <- brm(
  ## Especificamos el modelo
  formula = mean_torque_quad | se(sd_torque_quad) ~ jump_cmj_fuerza_peak,
...
saveRDS(quad_model, file = "R/bda/bm/quad_iso_torque_&_cmj_fuerza.RDS")
...
get_prior(formula = mean_torque_isquio | se(sd_torque_isquio) ~ jump_cmj_fuerza_peak, data = isqt_data)

isqt_model <- brm(
  ## Especificamos el modelo
  formula = mean_torque_isquio | se(sd_torque_isquio) ~ jump_cmj_fuerza_peak,
...
saveRDS(isqt_model, file = "R/bda/bm/isquios_iso_torque_&_cmj_fuerza.RDS")
