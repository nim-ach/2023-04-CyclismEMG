...
quad_data <- cyclist[
  ...
  list(
    ...
    jump_cmj_fuerza_peak = standardize(jump_cmj_fuerza_peak)
  )
]

quad_model <- brm(
  ## Especificamos el modelo
  formula = mean_torque_quad | se(sd_torque_quad, sigma = TRUE) ~ jump_cmj_fuerza_peak,
...
get_prior(formula = mean_torque_isquio | se(sd_torque_isquio, sigma = TRUE) ~ jump_cmj_fuerza_peak, data = isqt_data)

isqt_model <- brm(
  ## Especificamos el modelo
  formula = mean_torque_isquio | se(sd_torque_isquio, sigma = TRUE) ~ jump_cmj_fuerza_peak,
