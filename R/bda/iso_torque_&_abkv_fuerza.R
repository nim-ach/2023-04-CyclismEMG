...
quad_data <- cyclist[i = !is.na(iso_mean_torque_quad_der_raw),
                     j = list(
                       mean_torque_quad = mean(x = c(iso_mean_torque_quad_der_raw,
                                                     iso_mean_torque_quad_izq_raw)),
                       sd_torque_quad = mean(x = c(iso_sd_torque_quad_der_raw,
                                                 iso_sd_torque_quad_izq_raw)),
                       jump_abkv_fuerza_peak
                     ),
                     by = sujetos]

quad_model <- brm(
  ## Especificamos el modelo
  formula = mean_torque_quad | se(sd_torque_quad) ~ jump_abkv_fuerza_peak,
...
saveRDS(quad_model, file = "R/bda/bm/quad_iso_torque_&_abkv_fuerza.RDS")
...
get_prior(formula = mean_torque_isquio | se(sd_torque_isquio) ~ jump_abkv_fuerza_peak, data = isqt_data)

isqt_model <- brm(
  ## Especificamos el modelo
  formula = mean_torque_isquio | se(sd_torque_isquio) ~ jump_abkv_fuerza_peak,
...
saveRDS(isqt_model, file = "R/bda/bm/isquios_iso_torque_&_abkv_fuerza.RDS")
