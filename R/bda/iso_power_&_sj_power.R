...
quad_data <- cyclist[i = !is.na(iso_power_per_kg_quad_der_norm),
                     j = list(
                       mean_iso_power_quad = mean(c(iso_power_per_kg_quad_der_norm, iso_power_per_kg_quad_izq_norm)),
                       sd_iso_power_quad = sd(c(iso_power_per_kg_quad_der_norm, iso_power_per_kg_quad_izq_norm)),
                       jump_sj_power_mean
                     ),
                     by = sujetos]
...
quad_model <- brm(
  ...
  formula = mean_iso_power_quad | se(sd_iso_power_quad) ~ jump_sj_power_mean,

  ## Escogemos la distribución a priori de los coeficientes
  prior = prior(prior = student_t(3, 0, 10), class = b),
...
saveRDS(quad_model, file = "R/bda/bm/quad_iso_torque_&_sj_power.RDS")
...
saveRDS(isqt_model, file = "R/bda/bm/isquios_iso_torque_&_sj_power.RDS")
